...
10. В новой версии ядро также умеет выводить текст на экран.
11. Для этого заранее генерируется файл `font.inc`, в котором лежит bitmap-шрифт.
12. `main.c` ядра подключает `font.inc` и использует его для отрисовки символов во framebuffer.
13. Теперь вместо теста с цветными полосами можно вывести строку, например `Hello World!!!! &!@#$`, напрямую пикселями на экран.
---
...
Ядро получает возможность рисовать текст

В новой версии `mini-kernel/main.c` добавился вывод текста во framebuffer.

До этого ядро в основном проверяло работу нескольких процессоров через цветные горизонтальные полосы. Теперь появилась отдельная логика для вывода символов.
...
Он содержит bitmap-представление символов.

То есть каждый символ хранится не как обычная буква, а как набор битов:

```text
1 -> пиксель нужно закрасить
0 -> пиксель не нужно закрашивать
```

Размер одного символа задаётся константами:
...
Это означает, что каждый символ занимает прямоугольник:

```text
16 пикселей в ширину
24 пикселя в высоту
```

Например, если нужно вывести букву `A`, ядро берёт из `font.inc` массив битов для символа `A` и рисует его во framebuffer пиксель за пикселем.
...
берём символ 'H'
    ↓
находим bitmap символа 'H' в font.inc
    ↓
рисуем пиксели символа H
    ↓
переходим к следующему символу
    ↓
рисуем 'e', потом 'l', потом 'l', потом 'o'
```

В результате текст появляется на экране без использования UEFI `Print`, потому что после `ExitBootServices` UEFI-сервисы уже недоступны.
...
### Этап 17. Вместо полос ядро может выводить строку на экран

В новой версии код с запуском дополнительных процессоров может быть временно отключён или закомментирован, а вместо него в `main()` вызывается вывод строки.
...
```text
нарисовать строку "Hello World!!!! &!@#$"
начиная с координаты x = 100
и координаты y = 200
```

Важно: это уже не UEFI-вывод через `Print`.

Это собственный вывод ядра напрямую во framebuffer.

То есть ядро само вычисляет, какие пиксели нужно закрасить, и записывает цвет напрямую в видеопамять.

Если раньше экран использовался для проверки работы процессоров через полосы, то теперь экран используется ещё и как примитивный графический вывод.
...
# 2.6.1. `mini-kernel/font-gen.py`

## Архитектурно

`font-gen.py` — это вспомогательный скрипт, который генерирует bitmap-шрифт для ядра.

Он не выполняется внутри UEFI и не выполняется внутри kernel.
...
взять символы
↓
отрисовать каждый символ через ImageMagick
↓
получить bitmap каждого символа
↓
сохранить все символы в C-массив
↓
вывести результат в font.inc
```

## Пошагово `font-gen.py` делает

1. задаёт размер символа:

```python
w, h = 16, 24
```

2. формирует список печатных символов:

```python
printable = ascii_letters + digits + punctuation
```

3. для каждого ASCII-кода от `0` до `127` проверяет, является ли символ печатным;

4. если символ печатный, вызывает функцию `letter_bits`;

5. `letter_bits` вызывает внешнюю команду `convert`;

6. `convert` отрисовывает символ выбранным шрифтом;

7. результат преобразуется в набор байтов;

8. скрипт печатает C-код:
...
Поэтому один символ занимает 48 байт.
...
Это утитита из ImageMagick.

Если её нет, генерация `font.inc` упадёт.
...
Если команды нет, нужно установить ImageMagick:
...
В исходном варианте может использоваться шрифт:

```text
FiraCode-Nerd-Font-Reg
```

Если такого шрифта нет в системе, команда генерации упадёт.

В таком случае можно заменить шрифт в `font-gen.py`, например на:
...
```python
bits = letter_bits(chr(c), w, h, "FiraCode-Nerd-Font-Reg")
```
...
Посмотреть доступные шрифты можно так:

```bash
convert -list font | grep -i mono
```
...
## 2.9.3. Подключение `font.inc`

В новой версии `main.c` подключает файл:

```c
#include "font.inc"
```

Этот файл содержит bitmap-шрифт.

Он нужен для функций, которые рисуют текст на экране.

Если `processor_init.inc` нужен для запуска AP-процессоров, то `font.inc` нужен для вывода символов.

То есть теперь в `main.c` есть два важных include-файла:
...
```text
processor_init.inc — код для старта дополнительных процессоров;
font.inc           — данные шрифта для вывода текста.
```
...
## 2.9.4. `screen_set_pixel`

Функция `screen_set_pixel` рисует один пиксель во framebuffer.

Архитектурно она делает:

1. получает указатель на видеобуфер;
2. получает ширину и высоту экрана;
3. проверяет, что координаты не выходят за границы экрана;
4. вычисляет индекс пикселя в одномерном массиве;
5. записывает цвет в нужную ячейку framebuffer.

Идея такая:

```text
экран — это двумерная картинка
но framebuffer — это одномерный массив пикселей
```
...
то индекс пикселя считается так:

```text
index = y * width + x
```

После этого можно записать цвет:

```c
buffer[index] = color;
```

Эта функция нужна как базовый строительный блок.

Сама по себе она рисует только один пиксель, но на её основе можно рисовать:
...
## 2.9.4. `screen_draw_char`

Функция `screen_draw_char` рисует один символ.

Она использует данные из `font.inc`.
...
получаем bitmap символа
    ↓
проходим по всем пикселям символа
    ↓
если бит равен 1 — рисуем пиксель
    ↓
если бит равен 0 — ничего не рисуем
```

Каждый символ имеет размер:
...
То есть для одного символа функция проходит прямоугольник 16 на 24.

Для каждой точки внутри этого прямоугольника она проверяет соответствующий бит в массиве `font`.

Если бит установлен, вызывается:

```c
screen_set_pixel(...)
```

Так символ постепенно собирается на экране из отдельных пикселей.

Примерно так можно представить букву:
...
Единицы — это пиксели, которые нужно закрасить.

Нули — пустые места.

## 2.9.4. `screen_draw_string`

Функция `screen_draw_string` рисует строку.

Она не рисует строку сразу целиком.

Она идёт по строке символ за символом и для каждого символа вызывает:
...
нарисовать 'H'
↓
сдвинуть X вправо на ширину символа
↓
нарисовать 'e'
↓
сдвинуть X вправо
...
Если ширина символа равна:

```text
FONT_WIDTH = 16
```

то после каждого символа координата `x` увеличивается примерно на `16`.
...
```text
начать рисовать строку с позиции x = 100, y = 200
```

В итоге на экране появляется текст, нарисованный самим ядром.
...
# 2.9.X. Новая версия `main`

В старой версии `main()` использовался для проверки запуска нескольких процессоров.

Там BSP запускал AP-процессоры, а каждый процессор рисовал свою горизонтальную полосу.

В новой версии фокус сместился на проверку вывода текста.

Поэтому код запуска AP может быть временно закомментирован:
...
А вместо бесконечного цикла с полосами вызывается:

```c
screen_draw_string(&info->video, 100, 200, "Hello World!!!! &!@#$");
```

Это значит, что сейчас программа проверяет другой функционал:

```text
не запуск дополнительных CPU,
а собственный вывод текста во framebuffer.
```

Важно понимать: сам код запуска AP не обязательно удалён. Он может остаться в файле, но временно не использоваться.

То есть проект уже умеет две вещи:

```text
1. запускать дополнительные процессоры и рисовать полосы;
2. рисовать текст на экране через font.inc.
```

Просто в конкретной версии `main()` может быть активен один из этих сценариев.
...
`font.inc` — это сгенерированный файл, в котором лежит bitmap-шрифт для вывода текста.

Он похож по идее на `processor_init.inc`, но используется для другой цели.

Разница такая:

```text
processor_init.inc — содержит машинный код для запуска AP-процессоров;
font.inc           — содержит данные шрифта для рисования символов.
```

`font.inc` подключается в `main.c` ядра:

```c
#include "font.inc"
```

После подключения ядро получает массив:

```c
font
```

В этом массиве для каждого ASCII-символа хранится набор байтов.

Каждый бит внутри этих байтов означает, нужно ли рисовать пиксель символа.
...
2. Скрипт создаёт массив символов.

3. `main.c` подключает `font.inc`.

4. Функция `screen_draw_char` берёт нужный символ из массива `font`.

5. Если бит символа равен `1`, ядро рисует пиксель.

6. Если бит равен `0`, пиксель пропускается.

## Важный технический момент

`font.inc` должен быть сгенерирован до компиляции `main.c`.

Если файла нет, компилятор выдаст ошибку:

```text
font.inc: No such file or directory
```
...
### 3.12. Почему нужен `font.inc`

После `ExitBootServices` ядро уже не может использовать UEFI `Print`.

Поэтому, если мы хотим выводить текст, нужно реализовывать собственный вывод.

Самый простой способ — рисовать символы напрямую во framebuffer.

Для этого нужен bitmap-шрифт.

`font.inc` как раз и хранит такой bitmap-шрифт.

То есть вывод текста теперь работает так:
...
символ
↓
bitmap символа из font.inc
↓
пиксели
↓
framebuffer

### 3.13. Почему нужен ImageMagick

`font-gen.py` сам не умеет рисовать красивые символы.

Он вызывает внешнюю программу:

```text
convert
```

`convert` берёт символ, выбранный шрифт и превращает символ в bitmap.

Потом Python-скрипт превращает этот bitmap в C-массив.

Если `convert` не установлен, `font.inc` не сгенерируется.

### 3.14. Почему может возникнуть ошибка со шрифтом

Если в `font-gen.py` указан шрифт, которого нет в системе, например:

```text
FiraCode-Nerd-Font-Reg
```

то команда `convert` завершится ошибкой.

В таком случае нужно либо установить нужный шрифт, либо заменить его на доступный.
...
Проверить доступные шрифты можно командой:
...
Если менялся `font-gen.py` или нужно заново создать шрифт:

```bash
python3 font-gen.py > font.inc
```

Потом собираем kernel:

```bash
make
```

Если нужно полностью пересобрать с нуля:

```bash
rm -f kernel kernel.elf entry.o processor_init processor_init.inc font.inc
python3 font-gen.py > font.inc
make
```

После сборки нужно заново создать `kernel.inc`:
...
Затем пересобираем edk2-загрузчик:
...
Потом копируем новый `LabExitBs.efi` 
```bash
cp путь/до/LabExitBs.efi ~/uefi-bootloader/uefi_share/EFI/BOOT/LabExitBs.efi
```
